...
class PretrainViewMakerSystem(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.train_dataset, self.val_dataset = datasets.get_image_datasets(
            ...
            alternate_label=config.data_params.alternate_label
        )
    ...
    def normalize(self, imgs):
        ...
        if 'cifar' in self.config.data_params.dataset or 'celeba' in self.config.data_params.dataset: #'pcam' in self.config.data_params.dataset or 'ham' in self.config.data_params.dataset or 'pannuke' in self.config.data_params.dataset or 'drd' in self.config.data_params.dataset or 'idrid' in self.config.data_params.dataset:
            mean = torch.tensor([0.491, 0.482, 0.446], device=imgs.device)
            std = torch.tensor([0.247, 0.243, 0.261], device=imgs.device)
        elif 'mura' in self.config.data_params.dataset:
...
class PretrainExpertSystem(PretrainViewMakerSystem):
    ...
    def __init__(self, config):
        ...
        self.train_dataset, self.val_dataset = datasets.get_image_datasets(
            ...
            alternate_label=config.data_params.alternate_label,
            feat_dropout_prob=config.data_params.feat_dropout_prob
        )
        train_labels = self.train_dataset.dataset.targets
        self.train_ordered_labels = np.array(train_labels)
        self.model = self.create_encoder()
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % 200 == 0:
            # Log some example views.
            views_to_log = img.permute(
                0, 2, 3, 1).detach().cpu().numpy()[:10]
            wandb.log({"examples": [wandb.Image(
                view, caption=f"Epoch: {self.current_epoch}, Step {self.global_step},") for view in views_to_log]})
        return emb_dict
